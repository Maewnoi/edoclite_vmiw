...
class documents_admission_inside_deputy_signController extends Controller
{
    //

    public function index_0(){
        if(Auth::user()->level=='2'){
            return view('member.documents_admission_inside_deputy_sign.index');
        }else{
    ...
    public function detail($id){
        if(Auth::user()->level=='2'){
            ...
            return view('member.documents_admission_inside_deputy_sign.detail',compact('document_detail','userS'));
        }else{
            return redirect('member_dashboard')->with('error','คุณไม่มีสิทธิ์เข้าเมนูนี้ในระบบ !');
        }
    }
    ...
    public function understand(Request $request){
        ...
        if($request->sub3_sealid == 'ตีกลับ'){
            ...
            if($update_sub3_docs){
                ...
                return redirect()->route('documents_admission_inside_deputy_sign_all_0')->with('success',"ตีกลับเรียบร้อย");
            }else{
