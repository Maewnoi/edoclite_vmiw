    ...
    //
     ...
     public function index_0(){
        if(Auth::user()->level=='7'){
            ...
            // ->get();
            return view('member.documents_admission_work_inside_all.index');
        }else{
    ...
    public function detail($id){
        if(Auth::user()->level=='7'){
            ...
            if($document_detail->sub2_status == '0'){
                ...
                return view('member.documents_admission_work_inside_all.detail',compact('document_detail','sub3_doc_detail'))->with('success','อ่านเอกสารเรียบร้อย !');
            }else{
                return view('member.documents_admission_work_inside_all.detail',compact('document_detail','sub3_doc_detail'));
            }
        }else{
            return redirect('member_dashboard')->with('error','คุณไม่มีสิทธิ์เข้าเมนูนี้ในระบบ !');
        }
    ...
    public function respond(Request $request){
        if($request->bt_respond == 'respond_normal'){
             ...
             if($userS_0){
                 ...
                 //Generate ชื่อไฟล์
                 $name_gen_new = $insert_sub3_docs."_".$date_new;
                 // ดึงนามสกุลไฟล์
