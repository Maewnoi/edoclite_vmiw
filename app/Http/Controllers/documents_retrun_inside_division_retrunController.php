...
class documents_retrun_inside_division_retrunController extends Controller
{
    ...
    public function index(){
        if(Auth::user()->level=='4'){
            return view('member.documents_retrun_inside_division_retrun.index');
        }else{
    ...
    public function detail($id){
        if(Auth::user()->level=='4'){
            ...
            ->first();
            return view('member.documents_retrun_inside_division_retrun.detail',compact('document_retrun_inside_detail'));
        }else{
            return redirect('member_dashboard')->with('error','คุณไม่มีสิทธิ์เข้าเมนูนี้ในระบบ !');
        }
    }
            ...
            if($userS_0){
                ...
                if($full_path != null){
                    ...
                    if($update_documents_retrun && $update_documents_retrun_detail){
                        ...
                        return redirect()->route('documents_retrun_inside_division_retrun_all')->with('success',"ตอบการเอกสารเรียบร้อย");
                    }else{
