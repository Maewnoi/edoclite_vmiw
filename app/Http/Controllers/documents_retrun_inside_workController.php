...
class documents_retrun_inside_workController extends Controller
{
    //
    public function index(){
        if(Auth::user()->level=='7'){
            ...
            // ->get();
            return view('member.documents_retrun_inside_work.index');
        }else{
    ...
    public function detail($id){
        if(Auth::user()->level=='7'){
            ...
            ->where('docrt_id',$id)
            ->first();
            return view('member.documents_retrun_inside_work.detail',compact('document_retrun_inside_detail'));
        }else{
            return redirect('member_dashboard')->with('error','คุณไม่มีสิทธิ์เข้าเมนูนี้ในระบบ !');
        }
    }
