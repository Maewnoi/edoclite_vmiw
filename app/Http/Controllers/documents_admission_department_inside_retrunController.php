...
class documents_admission_department_inside_retrunController extends Controller
{
    //
    public function index(){
        if(Auth::user()->level=='5'){
            ...
            // ->get();
            return view('member.documents_admission_department_inside_retrun.index');
        }else{
    ...
    public function detail($id){
        if(Auth::user()->level=='5'){
            ...
            ->where('sub3_inspector_0', Auth::user()->id)
            ->first();
            return view('member.documents_admission_department_inside_retrun.detail',compact('document_detail'));
        }else{
            return redirect('member_dashboard')->with('error','คุณไม่มีสิทธิ์เข้าเมนูนี้ในระบบ !');
        }
    }
    ...
    public function understand(Request $request){
        ...
        if($userS_0){
            $update_sub3_docs = sub3_doc::where('sub3_id', $request->sub3_id)->update([
                'sub3_check_0'=>'1',
                'sub3_datetime_0'=>date('Y-m-d H:i:s'),
                'sub3_status'=>'1',
                'sub3_inspector_1'=>$userS_0->id,
